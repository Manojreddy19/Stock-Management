...
public class AdjustmentQueries {
	...
	protected String SELECT_ALL_ADJUSTMENTS_BY_CRITERIA = "SELECT * FROM tbl_adjustment_header where  AdjustmentType= :AdjustmentType AND (:adjustmentIdFlag=0 OR AdjustmentId= :AdjustmentId) AND (:statusFlag = 0 OR Status= :Staus) AND(:createdFromFlag = 0 OR  CreatedAt >= :CreatedFrom) AND (:createdToFlag = 0 OR CreatedAt <= :CreatedTo) ORDER BY Adjustmentid DESC LIMIT :NoOfRows OFFSET :LimitFrom"; 
