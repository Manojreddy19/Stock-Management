...
public class StockDaoImpl extends StockQueries implements StockDao {
	...
	public StockDetail getStockDetail(String productId, Long batchId) throws StockManagementException {
		...
		try {
			stock = namedParameterJdbcTemplate.queryForObject(SELECT_STOCK_DETAIL_BY_PID_AND_BID, params, new StockMapper());
		}
		catch(StockManagementException e) {
			e.printStackTrace();
		}
		if(stock==null) throw new StockManagementException("No Stock Found for product Id: " + productId + " and BatchId: " + batchId);
		return stock;
	}
