...
/// DONE FETCHING TOTAL RECORDS
  ...
  useEffect(()=>{
    axios.post("http://10.129.241.68:8080/api/getAdjustmentCount", parameters).then(res=>{setTotalRecords(res.data)})/*.then(console.log("totalRecords: ", totalRecords))*/;
  },[status])

  
  useEffect(()=>{
    axios.post("http://10.129.241.68:8080/api/getAdjustmentsByCriteria", params)
      .then(res=>{setDashData(res.data)})/*.then(console.log(dashData, params.noOfRows, params.page))*/;
  },[status, page])


  useEffect(()=>{
    axios.post("http://10.129.241.68:8080/api/getAdjustmentsByCriteria", formData)
      .then(res=>{setDashData(res.data);setStatus(formData.status)}).then(()=>{console.log("data", formData, dashData)});
  },[formData])
 ...
 setResponse({
  ...
  return (
    <div>
      ...
      <Pagination totalRecords={totalRecords} page={page} setPage={setPage}></Pagination>
    </div>
  );
};
