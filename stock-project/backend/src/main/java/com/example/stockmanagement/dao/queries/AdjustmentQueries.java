...
public class AdjustmentQueries {
	...
	protected String SELECT_ALL_ADJUSTMENTS_BY_CRITERIA = "SELECT * FROM tbl_adjustment_header WHERE (:AdjustmentType IS NULL OR AdjustmentType = :AdjustmentType) AND (:adjustmentIdFlag = 0 OR AdjustmentId = :AdjustmentId)AND (:statusFlag = 0 OR Status = :Status)AND (:createdFromFlag = 0 OR CreatedAt >= :CreatedFrom) AND (:createdToFlag = 0 OR CreatedAt <= :CreatedTo) ORDER BY AdjustmentId DESC LIMIT :NoOfRows OFFSET :LimitFrom";
